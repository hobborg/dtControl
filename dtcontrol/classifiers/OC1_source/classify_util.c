...
struct tree_node* read_hp (dtree) FILE* dtree;
{
    ...
    if (fscanf (dtree, "%[^' '] Hyperplane: Left = [", cur_node->label) != 1) return (NULL);

    for (i = 1; i < no_of_categories; i++)
        if (fscanf (dtree, "%d,", &cur_node->left_count[i]) != 1) return (NULL);
    if (fscanf (dtree, "%d], Right = [", &cur_node->left_count[no_of_categories]) != 1)
        return (NULL);
    for (i = 1; i < no_of_categories; i++)
        if (fscanf (dtree, "%d,", &cur_node->right_count[i]) != 1) return (NULL);
    if (fscanf (dtree, "%d]\n", &cur_node->right_count[no_of_categories]) != 1) return (NULL);
    ...
    while (TRUE)
    {
        ...
        if (c == 'x')
        {
            if ((fscanf (dtree, "[%d] +", &i)) != 1) error ("Invalid hyperplane equation.");
            if (i <= 0 || i > no_of_coeffs) error ("Invalid coefficient index in decision tree.");
    ...
    for (i = 1; i <= no_of_categories; i++)
    {
        cur_node->no_of_points += cur_node->left_count[i] + cur_node->right_count[i];
...
int read_header (dtree) FILE* dtree;
{
    ...
    if ((fscanf (dtree, "Dimensions: %d, Categories: %d\n", &no_of_dimensions, &no_of_categories)) != 2)
        return (0);
    if ((fscanf (dtree, "Program Parameters: Iter: %d, Order: %s", &no_of_iterations, order_of_improvement)) != 2)
        return (0);
    return (1);
}
