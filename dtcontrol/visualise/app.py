...
@app.route("/clear")
def clear():
    global computed_configs, selected_computation_result
    computed_configs = []
    selected_computation_result = None
...
def select():
    ...
    selected_computation_result = computed_configs[int(request.form['runConfigIndex']) - 1]
    return jsonify(success=True)
...
def simroute():
    ...
    computed_configs.append(([classi[0]], saved_tree, minBounds, maxBounds, stepSize, numVars, numResults))
    return jsonify(classi[3].get_stats())
