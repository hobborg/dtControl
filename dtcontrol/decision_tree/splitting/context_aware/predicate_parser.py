...
class PredicateParser:
    ...
    def parse_single_predicate(cls, single_predicate, logger, debug=False):
        ...
        """
        logger.root_logger.info(f"Predicate to parse: {single_predicate}")
        try:
                ...
                # Type: c_i --> coef
                    ...
                    if not all_interval_defs.__contains__(var):
                        ...
                        if isinstance(check_interval, sp.FiniteSet) or check_interval == infinity:
                            ...
                            logger.root_logger.critical(
                                f"Aborting: Invalid interval for a coefficient declared. Only finite or (-Inf, Inf) allowed. Coefficient {var} with invalid interval {check_interval} from predicate: {single_predicate}")
                            raise RicherDomainPredicateParserException()
                else:
                    logger.root_logger.critical(
                        f"Aborting: One symbol inside one predicate does not have a valid structure. Column refs only with x_i. Coefs only with c_i. Invalid symbol: '{var}' inside predicate {single_predicate}")
                    raise RicherDomainPredicateParserException()
            ...
            if term.atoms(AppliedUndef):
                logger.root_logger.critical(
                    f"Aborting: One predicate contains an undefined function. Undefined function: {term.atoms(AppliedUndef)}. Invalid predicate: {single_predicate}")
                raise RicherDomainPredicateParserException()
            # Hidden edge case2: No symbols to reference columns used.
            # e.g. c_0 >= 1; c_0 in {5,7}
            elif not column_interval:
                logger.root_logger.critical(
                    f"Aborting: One predicate does not contain variables to reference columns. Invalid predicate: {single_predicate}")
                raise RicherDomainPredicateParserException()
            # Hidden edge case3: Term evaluates to zero.
            # e.g. 3-1.5*2 <= 0
            elif term == 0 or term.evalf() == 0:
                logger.root_logger.critical(
                    f"Aborting: One predicate does evaluate to zero. Invalid predicate: {single_predicate}")
                raise RicherDomainPredicateParserException()
            # Hidden edge case4: Invalid states for important key variables reached.
                ...
                logger.root_logger.critical(f"Aborting: One predicate does not have a valid structure. Invalid predicate: {single_predicate}")
                raise RicherDomainPredicateParserException()
            ...
            for var in all_interval_defs:
                ...
                if re.match(r"x_\d+", str(var)):
                    ...
                    logger.root_logger.critical(
                        f"Aborting: Invalid symbol in interval definition without symbol usage in the term found. Invalid symbol(s): {all_interval_defs} inside predicate: {single_predicate}")
                    raise RicherDomainPredicateParserException()
    ...
    def preprocess_single_predicate(cls, single_predicate, logger):
        ...
        logger.root_logger.critical(
            f"Aborting: One predicate did not contain any relation. Invalid predicate: {single_predicate}")
        raise RicherDomainPredicateParserException()
    ...
    def get_domain_knowledge(cls, debug=False,
        ...
        if input_line[0].startswith("#UNITS"):
            ...
            logger.root_logger.info(f"Units found: {converted_units}")

        logger.root_logger.info(
            f"Reading input file containing domain knowledge given by user. Found {len(input_line)} predicate(s).")

        for single_predicate in input_line:
            logger.root_logger.info(
                f"Processing user domain knowledge {input_line.index(single_predicate) + 1} / {len(input_line)}.")

            # Parse single predicate
            parsed_predicate = cls.parse_single_predicate(single_predicate, logger, debug=debug)

            logger.root_logger.info(
                f"Parsed predicate {input_line.index(single_predicate) + 1} / {len(input_line)} successfully. Result: {str(parsed_predicate)}")
    ...
    def get_predicate(cls, debug=False, input_file_path=r"dtcontrol/decision_tree/splitting/context_aware/input_data/input_predicates.txt"):
        ...
        if not input_line:
            ...
            logger.root_logger.info(
                f"Reading input file containing predicate(s) given by user. Found {len(input_line)} predicate(s).")

        # output list containing all parsed predicates
        output = []
        for single_predicate in input_line:
            logger.root_logger.info(f"Processing user predicate {input_line.index(single_predicate) + 1} / {len(input_line)}.")
    ...
    def parse_user_interval(cls, user_input, logger):
        ...
        logger.root_logger.info(f"User interval to process: {user_input}")
        ...
        if not user_input.strip():
            ...
            logger.root_logger.critical(f"Aborting: interval starts with an invalid char. Invalid interval: {user_input}")
            raise RicherDomainPredicateParserException()
        elif user_input.strip()[-1] != '}' and user_input.strip()[-1] != ')' and user_input.strip()[
            -1] != ']':
            logger.root_logger.critical(f"Aborting: interval ends with an invalid char. Invalid interval: {user_input}")
            raise RicherDomainPredicateParserException()
        ...
        for interval in user_input:
            ...
            if interval[0] == "{":
                ...
                if interval[-1] == "}":
                    ...
                    for var in unchecked_members:
                        try:
                            ...
                            logger.root_logger.critical(f"Aborting: Invalid member found. Invalid interval: {user_input}")
                            raise RicherDomainPredicateParserException()
                        if tmp == sp.nan:
                            logger.root_logger.critical(f"Aborting: Invalid NaN member found. Invalid interval: {user_input}")
                            raise RicherDomainPredicateParserException()
                        elif tmp == sp.S.Infinity or tmp == sp.S.NegativeInfinity:
                            logger.root_logger.critical(f"Aborting: Infinity is an invalid member. Invalid interval: {user_input}")
                            raise RicherDomainPredicateParserException()
                        elif isinstance(tmp, sp.Number):
                            checked_members.append(tmp)
                        else:
                            logger.root_logger.critical(
                                f"Aborting: Invalid member found in finite interval. Invalid member: {tmp} Invalid interval: {user_input}")
                            raise RicherDomainPredicateParserException()
                    # Edge case: if no member is valid --> empty set will be returned.
                    out = sp.FiniteSet(*checked_members)
                    if out == sp.EmptySet:
                        logger.root_logger.critical(f"Aborting: Invalid empty interval found. Invalid interval: {user_input}")
                        raise RicherDomainPredicateParserException()
                    interval_list.append(out)
                else:
                    # Interval starts with { but does not end with }
                    logger.root_logger.critical(f"Aborting: Invalid char at end of interval found. Invalid interval: {user_input}")
                    raise RicherDomainPredicateParserException()
                ...
                if len(tmp) > 2:
                    logger.root_logger.critical(f"Aborting: Zoo many numbers inside an interval. Invalid interval: {user_input}")
                    raise RicherDomainPredicateParserException()
                try:
                    ...
                    if a == sp.nan or b == sp.nan:
                        logger.root_logger.critical(
                            f"Aborting: invalid NaN interval boundary found. Invalid interval: {user_input}")
                        raise RicherDomainPredicateParserException()
                except Exception:
                    logger.root_logger.critical(f"Aborting: Invalid member found inside interval. Invalid interval: {user_input}")
                    raise RicherDomainPredicateParserException()
                else:
                    if isinstance(a, sp.Number) and isinstance(b, sp.Number):
                        ...
                        if interval[-1] == ")":
                            ...
                            logger.root_logger.critical(
                                f"Aborting: interval ends with an invalid char. Invalid interval: {user_input}")
                            raise RicherDomainPredicateParserException()
                        ...
                        logger.root_logger.critical(
                            f"Aborting: Invalid member found inside interval. Invalid interval: {user_input}")
                        raise RicherDomainPredicateParserException()
            else:
                logger.root_logger.critical(f"Aborting: Invalid char found inside interval. Invalid interval: {user_input}")
                raise RicherDomainPredicateParserException()
        ...
        logger.root_logger.info(f"Finished processing user interval. Result: {final_interval}.")
    ...
    def parse_user_string(cls, user_input):
        ...
        for single_predicate in predicate_list:
            logger.root_logger.info(f"Processing user predicate {predicate_list.index(single_predicate) + 1} / {len(predicate_list)}.")

            # Parse single predicate
            parsed_predicate = cls.parse_single_predicate(single_predicate, logger, debug=False)

            logger.root_logger.info(
                f"Parsed predicate {predicate_list.index(single_predicate) + 1} / {len(predicate_list)} successfully. Result: {parsed_predicate}")
            output.append(parsed_predicate)
